...
argp.add_argument(
    ...
    type=int,
    default=2,
    help="control mode: 1 Pos&Vel, 2 Torque+PD, 3 Vel",
)
...
if args.control_mode == 1:
    ...
    status = easy_controllers.run_controller(
        robot, make_controller(q1), dt=0.01, max_time=10
    )
...
if args.control_mode == 2:
    ...
    status = easy_controllers.run_controller(
        robot, make_controller(qhome), dt=0.01, max_time=10.0
    )
    print("status", status)

    status = easy_controllers.run_controller(
        robot, make_controller(q1), dt=0.01, max_time=10.0
    )
    ...
    status = easy_controllers.run_controller(
        robot,
        easy_controllers.GoJointConfigurationVelocity(
            qgoal=qhome, error_goal=0.01, max_v=0.5, kd=1.5
        ),
        dt=0.01,
        max_time=10.0,
    )
    ...
    status = easy_controllers.run_controller(
        robot,
        easy_controllers.GoJointConfigurationVelocity(
            qgoal=q1, error_goal=0.01, max_v=0.5, kd=1.5
        ),
        dt=0.01,
        max_time=10.0,
    )
