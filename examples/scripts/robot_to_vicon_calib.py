...
if generate_trajectory:
    ...
    status = easy_controllers.run_controller(
        robot, controller, dt=0.005, max_time=60, callback=state_recorder.callback
    )
    ...
    for q in waypoints:
        ...
        T = pin_model.framePlacement(
            np.array(s.q), frame_id_flange, update_kinematics=True
        )
        robot_ts.append(T)
